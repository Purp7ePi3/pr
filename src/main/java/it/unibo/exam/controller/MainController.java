 ...
 * Updated version with Easter Egg detection when all minigames are completed.
 */
public class MainController {
    ...
    /**
     * Check win condition and Easter Egg trigger.
     */
    private void checkWin() {
        ...
        if (gameState.getPlayer().allRoomsCompleted(totalPuzzleRooms)) {
            if (!easterEggTriggered) {
                ...
                LOGGER.info("All minigames completed! Triggering Easter Egg...");
                triggerEasterEgg();
            }
        }
    }
    ...
    /**
     * Triggers the Easter Egg battle when all minigames are completed.
     */
    private void triggerEasterEgg() {
        SwingUtilities.invokeLater(() -> {
            int choice = JOptionPane.showConfirmDialog(
                parentFrame,
                "🎉 Congratulations! You've completed all minigames!\n\n" +
                "🥚 You've unlocked an Easter Egg!\n\n" +
                "Would you like to play the secret Pokémon-style battle?\n" +
                "Students vs Pianini!",
                "Easter Egg Unlocked!",
            ...
            if (choice == JOptionPane.YES_OPTION) {
                startEasterEggBattle();
            }
        });
    }

    /**
     * Starts the Easter Egg Pokémon battle.
     */
    private void startEasterEggBattle() {
        try {
            // Import the Pokemon battle minigame class
            it.unibo.exam.model.entity.minigame.PokemonBattleMinigame battleGame = 
                new it.unibo.exam.model.entity.minigame.PokemonBattleMinigame();
            
            battleGame.start(parentFrame, new it.unibo.exam.model.entity.minigame.MinigameCallback() {
                @Override
                public void onComplete(boolean success, int timeSeconds) {
                    handleEasterEggComplete(success, timeSeconds);
                }
            });
            
        } catch (Exception e) {
            LOGGER.severe("Error starting Easter Egg battle: " + e.getMessage());
            e.printStackTrace();
    ...
    /**
     * Handles the completion of the Easter Egg battle.
     * 
     * @param success whether the battle was won
     * @param timeSeconds time taken for the battle
     */
    private void handleEasterEggComplete(boolean success, int timeSeconds) {
        SwingUtilities.invokeLater(() -> {
            if (success) {
                JOptionPane.showMessageDialog(parentFrame,
                    "🏆 Victory! Students have defeated the Pianini!\n\n" +
                    "You are now the ultimate University Escape champion!\n" +
                    "Battle time: " + timeSeconds + " seconds",
                    "Battle Won!",
                    JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(parentFrame,
                    "💀 Defeat! The Pianini were too strong...\n\n" +
                    "Better luck next time, young trainer!\n" +
                    "Battle time: " + timeSeconds + " seconds",
                    "Battle Lost",
