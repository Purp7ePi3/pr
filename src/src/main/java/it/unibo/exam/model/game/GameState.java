        ...
        for (int i = 1; i < rooms.size(); i++) {
            ...
            if (room.getRoomType() == RoomGenerator.PUZZLE_ROOM) {
                try {
                    ...
                    npc.setPositionX(environmentSize.getX() / 2 - offset); // Center with slight offset
                    npc.setPositionY(environmentSize.getY() / 2 - offset);

                    room.attachNpc(npc);
                } catch (final IllegalArgumentException e) {
                    // Log the error and continue with next room instead of crashing
